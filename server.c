...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // store request line parts into variables
            char* method = strtok(linecopy, " \r\n");
            char* reqtarget = strtok(NULL, " \r\n");
            char* version = strtok(NULL, " \r\n");
            
            // declare absolutepath
            char absolutepath[strlen(reqtarget)];
            int absolutepathlength = 0;
            
            // ensure request line has 2 spaces
            int sp = 0;
            ...
            // ensure there are only two spaces in the request line
            if(sp != 2)
            {
                error(400);
                continue;
            }
            // ensure request target begins with backslash
            if(reqtarget[0] != '/')
            {
                error(400);
                continue;
            }
            // ensure there is no quotes in the request target
            ...
            // if method is not GET respond with 405 error
            ...
            // if request-target doesn't begin with slash 501
            if(reqtarget[0] != '/')
            {
                error(501);
                continue;
            ...
            // if request target contains a quote, 400 error
            if(strchr(reqtarget, '"') != NULL)
            {
                error(400);
                continue;
            ...
            // if version is not HTTP/1.1 error 505
            ...
            // determine the absolutepath here
            // if there is no q, reqtarget is absolutepath
            // if absolutepath does not contain . respond with 501
            if(strchr(reqtarget, '?') == NULL)
            {
                
                strcpy(absolutepath, reqtarget);

                if(strrchr(reqtarget, '.') == NULL)
                {
                    error(501);
                    continue;
                }
            ...
            // absolutepath is string left of ? mark
            
            if(strchr(reqtarget, '?') != NULL)
            {
                
                absolutepathlength = strlen(reqtarget) - strlen(strchr(reqtarget, '?'));
                strncpy(absolutepath, reqtarget, absolutepathlength);
                
                 absolutepath[absolutepathlength] = '\0';
         
            }
             if(strrchr(absolutepath, '.') == NULL)
                {
                    error(501);
                    continue;
                }
            ...
            // store query string into variable called quest
            char* quest = strchr(reqtarget, '?');
            
            char query[strlen(reqtarget)]; // "TODO";
            // check if there is a query parameter
            // and if no query, set query to null
            if(quest == NULL)
            {
                query[0] = '\0';
            }
            // if there is a query parameter, copy to query string
            else
            {
                strcpy(query, quest+1);
            }
           
            // TODO: concatenate root and absolute-path
            // copy root into a temp variable
            char root_temp[strlen(root)];
            
            char path[strlen(root) + strlen(absolutepath)]; //= "TODO";
            // copy root into temp variable root_temp
            strcpy(root_temp, root);
            
            // concatenate abolutepath to root_temp, store in destination variable
            char* dest = strcat(root_temp, absolutepath);
            
            // copy destination into path
            strcpy(path, dest);
            
            // TODO: ensure path exists
            int result;
            result = access(path, F_OK);
            // if result is not zero, then respond with 404
            if(result != 0)
            {
                error(404);
                continue;
            ...
            // store extenstion into temp variable
            char* ext = strrchr(path, '.');
            // get size of extension
            int extlength = strlen(ext);
            char extension[extlength];
            
            // copy ext to extension
